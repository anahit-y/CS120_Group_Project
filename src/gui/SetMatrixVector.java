...
public class SetMatrixVector extends JButton {
    ...
    public void openTwoWindows(int rows, int col, String type, String operationName) {
        this.operationName = operationName;
        int width = col * 40 + 40;  // Cell width
        int height = rows * 40 + 40; // Cell height adjusted for text fields

        JFrame frameA = createMatrixWindow(type + " A Input", rows, col, width, height);
        JFrame frameB = createMatrixWindow(type + " B Input", rows, col, width, height);
        ...
        calculateButton.addActionListener(e -> {
            if (type.equals("Vector")) {
                ...
                double[] vectorB = extractVector(frameB, rows);
                Calculate calculate = new Calculate(vectorA, vectorB, operationName);
                calculate.performCalculation();
            } else {
                Calculate calculate = new Calculate(frameA, frameB, operationName, rows, col, rows, col);
                calculate.performCalculation();
            }
        });
