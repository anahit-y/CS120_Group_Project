...
// methods inherited from Matrix interface
    ...
    public SquareMatrix rowEchelonForm() {
        SquareMatrix result = new SquareMatrix(this);
        for (int i = 0; i < this.getSize(); i++) {
            if (result.getElementAt(i, i) == 0) {
                for (int j = i + 1; j < this.getSize(); j++) {
                    if (result.getElementAt(j, i) != 0) {
                        double[] temp = result.matrix[i];
                        result.matrix[i] = result.matrix[j];
                        result.matrix[j] = temp;
                        break;
                    }
                }
            }
            for (int j = i + 1; j < this.getSize(); j++) {
                double factor = result.getElementAt(j, i) / result.getElementAt(i, i);
                for (int k = i; k < this.getSize(); k++) {
                    result.matrix[j][k] -= factor * result.getElementAt(i, k);
                }
            }
        }
        return result;
    ...
    public SquareMatrix reducedRowEchelonForm() {
        SquareMatrix result = this.rowEchelonForm();
        for (int i = this.getSize() - 1; i >= 0; i--) {
            if (result.getElementAt(i, i) != 0) {
                for (int j = i - 1; j >= 0; j--) {
                    double factor = result.getElementAt(j, i);
                    for (int k = i; k < this.getSize(); k++) {
                        result.matrix[j][k] -= factor * result.getElementAt(i, k);
                    }
                }
